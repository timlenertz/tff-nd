...
template<std::size_t Dim, bool Mutable, typename Frame_format> class ndarray_opaque_view;
	
namespace detail {
	template<std::size_t Dim, bool Mutable, typename Frame_format>
	using ndarray_opaque_view_base_ = ndarray_view<Dim + 1, std::conditional_t<Mutable, byte, const byte>>;
}
